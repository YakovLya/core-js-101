...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (bottom1 > rect2.top && right1 > rect2.left
      && rect1.top < bottom2 && rect1.left < right2) { return true; }
  if (bottom2 > rect1.top && right2 > rect1.left
      && rect2.top < bottom1 && rect2.left < right1) { return true; }
  return false;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  if (a < b) { return `${(isStartIncluded ? '[' : '(') + a}, ${b}${isEndIncluded ? ']' : ')'}`; }
...
function isBracketsBalanced(str) {
  const stack = [];
  for (let i = 0; i < str.length; i += 1) {
    if (str[i] === '(' || str[i] === '[' || str[i] === '{' || str[i] === '<') { stack.push(str[i]); } else {
      const lastBracket = stack.pop();
      if ((str[i] === ')' && lastBracket !== '(')
          || (str[i] === ']' && lastBracket !== '[')
          || (str[i] === '}' && lastBracket !== '{')
          || (str[i] === '>' && lastBracket !== '<')) { return false; }
    }
  }
