...
class CssBuilder {
  constructor() {
    ...
    this.haveId = false;
    this.havePseudoElement = false;
    this.order = 0;
  }

  element(value) {
    if (this.haveElement) { throw new Error('Element, id and pseudo-element should not occur more then one time inside the selector'); }
  ...
  id(value) {
    if (this.haveId) { throw new Error('Element, id and pseudo-element should not occur more then one time inside the selector'); }
  ...
  pseudoElement(value) {
    if (this.havePseudoElement) { throw new Error('Element, id and pseudo-element should not occur more then one time inside the selector'); }
    if (this.order > 6) { throw new Error('Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'); }
    this.value += `::${value}`;
    this.havePseudoElement = true;
    this.order = 6;
    return this;
  }
...
const cssSelectorBuilder = {
  ...
  combine(selector1, combinator, selector2) {
    const selector = new CssBuilder();
    selector.combine(selector1, combinator, selector2);
    return selector;
  },
};
